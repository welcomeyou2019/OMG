...
class TUDataset(TUDataset_):
    ...
    @property
    def num_tasks(self):
        return 2 if self.name != "IMDB-MULTI" else 3
...
class NodeDeleted_imdb(Graph2Subgraph):
    def to_subgraphs(self, data):
...
class EdgeDeleted_imdb(Graph2Subgraph):
    def to_subgraphs(self, data):
        # remove one of the bidirectional index
        data.x = degree(data.edge_index[0], data.num_nodes, dtype=torch.long)
        data.x = F.one_hot(data.x, num_classes=136).to(torch.float)
...
def policy2transform_imdb(policy: str, num_hops, process_subgraphs=lambda x: x, pbar=None):
    if policy == "edge_deleted":
        return EdgeDeleted_imdb(process_subgraphs=process_subgraphs, pbar=pbar)
    elif policy == "node_deleted":
        return NodeDeleted_imdb(process_subgraphs=process_subgraphs, pbar=pbar)
